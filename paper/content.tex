...
Internet" ~\cite{cloud1}. Cloud Computing provides a flexible IT 
...
without installing the software on the local machine ~\cite{cloud2}. 
...
when compared to others ~\cite{cloud3}.
...
organization in the community ~\cite{cloud3}.
...
provides very less security and it cost less ~\cite{cloud3}.
...
that enables data and application portability ~\cite{cloud3}.
...
the user operates ~\cite{zissis2012addressing}.
...
areas of cloud security concerns ~\cite{gartner}.
...
and security certifications ~\cite{gartner}. 
...
obey local privacy requirements on behalf of their customers ~\cite{gartner}.
...
and their applications ~\cite{vmware}. Physical data center should be properly secured 
...
Host machine protection should include ~\cite{diversity} intrusion detection system 
...
for proper mitigation strategies ~\cite{winkler}. Provider should enforce strong 
...
disaster recovery ~\cite{vmware}. All API's through which the customers access
the cloud resources with SSL, recommended to provide the secure
communication over Internet has to be encrypted ~\cite{diversity}.
...
virtual machines ~\cite{diversity}.
...
and the best use different passwords for different services ~\cite{redpaper}.
...
of sudden data loss in the cloud services ~\cite{diversity}.
...
to remotely wipe any data stored on a computer ~\cite{diversity}.
...
proper security mechanisms are in place ~\cite{diversity}.
...
Control (DAC) and Role Based Access Control (RBAC) ~\cite{accesscntrl}.
...
and efficient management ~\cite{cloud3}. 
...
of attributes of the user key matches the attributes of the ciphertext ~\cite{poland}.
